...
OrderResult MatchingEngine::cancel_order(const Symbol &symbol,
                                         OrderId order_id) {
...
OrderResult MatchingEngine::replace_order(const Symbol &symbol,
                                          OrderId order_id, Price new_price,
                                          Quantity new_qty) {
  OrderBook *book = get_book(symbol);
  if (!book) {
    OrderResult result;
...
bool MatchingEngine::process_one() {
  ...
  case MessageType::ReplaceOrder:
    replace_order(msg.symbol, msg.order_id, msg.new_price, msg.new_qty);
    break;
  }
...
void MatchingEngine::stop() { running_ = false; }
...
void MatchingEngine::reset_stats() { stats_ = {}; }
...
void EngineRunner::start() {
  running_ = true;
  // Threading not implemented yet - users should call engine.process_one()
  // directly
}
...
bool EngineRunner::is_running() const { return running_; }
