...
TEST(cancel_already_filled_order) {
    ...
    book->add_order(Order(fresh_id(), sym, Side::Sell, 10000, 100));  // Fills the bid
...
TEST(ioc_fills_available_and_cancels_rest) {
    ...
    Order ioc(fresh_id(), sym, Side::Buy, 10000, 100, OrderType::Limit, TimeInForce::ImmediateOrCancel);
    auto result = book->add_order(ioc);
    
    ASSERT_TRUE(result.success);
    ASSERT_EQ(result.filled_qty, 30u);
    ASSERT_EQ(book->best_bid(), INVALID_PRICE);  // Remainder cancelled, not resting
}
...
TEST(fok_fills_completely_or_rejects) {
    ...
    Order fok(fresh_id(), sym, Side::Buy, 10000, 100, OrderType::Limit, TimeInForce::FillOrKill);
