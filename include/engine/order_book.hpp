...
public:
  ...
  OrderResult replace_order(OrderId order_id, Price new_price,
                            Quantity new_qty);
...
private:
  ...
  // Emit trade event
  void emit_trade(const Order &buy, const Order &sell, Price price,
  ...
  void emit_execution_report(const Order &order, Quantity filled,
                             Price avg_price);
